...
//重置搜索条件
function Reset() {
    $(".contentConditions").find("input[type='text']").attr("value", "");;
...
function AlertShow(width, height, url) {
    ...
    $("iframe").css("background-color", "#161523");
}
//function Exit() {
//    if (confirm("确定退出系统吗？")) {
...
//}
function showDown() {
    var style = Math.floor($("body").height() / 2) + "px auto";
    $("#showDiv").height($("body").height());
    $("#imgdiv").css("margin", style);
    $("#showDiv").show();
}
function closeDown() {
    $("#showDiv").hide();
