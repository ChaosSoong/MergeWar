...
(function() {
    ...
    d.prototype._bindCircle = function() {
        ...
        var l = function(q) {
            ...
            k = null;
            //console.log(saveData);//画圆的部分
            $.post('add.php', { type: type, lat: saveData.center.lat, lng: saveData.center.lng, radius: saveData.radius, style: styleOptions }, function(data) {
                // console.log(data);
            });
            o.disableEdgeMove();
    ...
    d.prototype._bindPolylineOrPolygon = function() {
        ...
        var g = function(p) {
            ...
            for (var i = 0; i < saveData.length - 1; i++) {
                var tmp1 = [];
                tmp1.push(saveData[i].lng);
                tmp1.push(saveData[i].lat);
                tmp.push(tmp1);
            };
            // console.log("折线");
            // console.log(tmp);
            ...
            //map.clearOverlays();
            polygons = [];
            var t = hcq(tmp, distance);
            // console.log(t);
            // var left = [];
            // var right = [];
            // if (tmp.length >= 2) {
            //     // 第一个节点
            //     right.push(new BMap.Point(duandian(tmp[0], tmp[1], 1)[0], duandian(tmp[0], tmp[1], 1)[1]));
            //     left.unshift(new BMap.Point(duandian(tmp[0], tmp[1], 1)[2], duandian(tmp[0], tmp[1], 1)[3]));
            //     // 中间节点
            //     for (var i = 0; i < tmp.length - 2; i++) {
            //         right.push(new BMap.Point(AngleBisector(tmp[i], tmp[i + 1], tmp[i + 2], 1)[0], AngleBisector(tmp[i], tmp[i + 1], tmp[i + 2], 1)[1]));
            //         left.unshift(new BMap.Point(AngleBisector(tmp[i], tmp[i + 1], tmp[i + 2], 1)[2], AngleBisector(tmp[i], tmp[i + 1], tmp[i + 2], 1)[3]));
            //     }
            //     // 最后一个节点
            //     right.push(new BMap.Point(duandian(tmp[tmp.length - 1], tmp[tmp.length - 2], 1)[0], duandian(tmp[tmp.length - 1], tmp[tmp.length - 2], 1)[1]));
            //     left.unshift(new BMap.Point(duandian(tmp[tmp.length - 1], tmp[tmp.length - 2], 1)[2], duandian(tmp[tmp.length - 1], tmp[tmp.length - 2], 1)[3]));

            // }
            // var polygons = GetBufferEdgeCoords(tmp.join(';'), 1);
            // polygons = right.concat(left);
            for (var i = 0; i < t[0].length; i++) {
                polygons.push(new BMap.Point(t[0][i], t[1][i]));
            }

            // map.addOverlay(new BMap.Polygon(polygons, { strokeColor: "blue", strokeWeight: 2, strokeOpacity: 0.5 }));
            var ptsIn = [];
            // console.log(polygons);
            for (var jjjj = 0; jjjj < pts.length; jjjj++) {
                var result = BMapLib.GeoUtils.isPointInPolygon(markers[jjjj].point, new BMap.Polygon(polygons));
                if (result) {
                    ptsIn.push(markers[jjjj]);
                } else {
                    continue;
                }
            }
    ...
    d.prototype._bindRectangle = function() {
        ...
        var g = function(p) {
            ...
            for (var ii = 0; ii < saveData.length; ii++) {
                var tmp1 = [];
                tmp1.push(saveData[ii].lng);
                tmp1.push(saveData[ii].lat);
                tmp.push(tmp1);
            };
            $.post('add.php', { parent: parent, type: type, data: tmp, style: styleOptions }, function(data) {
                //console.log(data);
