   ...
   public class SystemDAL
   {
       ...
       public PagedList<AreaInfo> GetAreaManage(Dictionary<string, string> dic)
        {
            ...
            try
            {
                ...
                if (CityType == "2")
                {
                    sql = "SELECT * FROM ( SELECT ROW_NUMBER () OVER (ORDER BY ai.ID DESC) PageNum,ai.ID,ai.Area,ai.CreateTime,ai.City_code FROM AreaInfo ai " + whereSql + " ) temp WHERE temp.PageNum BETWEEN " + ((pageIndex - 1) * pageSize + 1) + " AND " + pageIndex * pageSize;
       ...
       public PagedList<PoliceInfo> GetPoliceInfoList(Dictionary<string, string> dic)
       {
           ...
           try
           {
               ...
               sql = "SELECT * FROM (SELECT ROW_NUMBER () OVER (ORDER BY p.CreateTime DESC) PageNum, p.*,a.Area FROM PoliceInfo p LEFT JOIN  AreaInfo a ON a.ID = p.AId " + whereSql + " ) temp WHERE temp.PageNum BETWEEN " + ((pageIndex - 1) * pageSize + 1) + " AND " + pageIndex * pageSize;
       ...
       /// <returns></returns>
       public bool GetPoliceInfo(string Name, int Id, int Aid)
       {
